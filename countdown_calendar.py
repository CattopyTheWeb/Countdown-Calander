...
c.create_text(100, 50, anchor='w', fill='orange',
              font='Arial 28 bold', text='My Countdown Calendar')
events = get_events()
today = date.today()

c.create_text(100, 100, anchor='w', fill='green',
              font='Arial 28 bold', text='Today is: ' + str(today))
vertical_space = 150
...
for event in events:
    ...
    if (int(days_until)) <= 7:
        text_col = 'red'
        fontty = 'Arial 28 bold underline'
    else:
        text_col = 'lightblue'
        fontty = 'Arial 28 bold'
    c.create_text(100, vertical_space, anchor='w', fill=text_col,
                  font=fontty, text=display)
